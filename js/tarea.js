class Tarea {
    constructor(id, texto, estado, contenedor) {
        ...
        this.texto = texto;
        this.DOM = null;
        this.editando = false; //esta propiedad, determina el estado de la tarea, si estamos o no editándola 
    ...
    borrarTarea() {
        fetch(`https://api-todo-fullstack-0ut2.onrender.com/tareas/borrar/${this.id}`,{
            method : "DELETE"
        })
    ...
    toggleEstado() {
        return new Promise((ok, ko) => {
            fetch(`https://api-todo-fullstack-0ut2.onrender.com/tareas/actualizar/${this.id}/2`,{
                method : "PUT"
            })
            .then(respuesta => respuesta.json())
            .then(({resultado,error}) => {
    ...
    async editarTexto() {
        if (this.editando) {
            ...
            if(posibleTexto != "" && posibleTexto != this.texto){
                let {error} = await fetch(`https://api-todo-fullstack-0ut2.onrender.com/tareas/actualizar/${this.id}/1`,{
                    method : "PUT",
            ...
            //activar la interface de edición
